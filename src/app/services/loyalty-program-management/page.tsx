...
export async function generateMetadata(): Promise<Metadata> {
  const page: any = await findBy("pages", "slug", "service-management");
  return {
    title: page?.seoTitle || "Loyalty Management | QBF Consulting",
    description:
      page?.seoDescription || "Loyalty program management services from QBF Consulting.",
  };
}

export default async function ServiceItemPage() {
  const page: any = (await findBy("pages", "slug", "service-management")) || {};
  const heroTitle = page.heroTitle || "Loyalty Program Management.";
  const heroSubtitle =
    page.heroSubtitle ||
    "Ongoing optimisation and performance management to ensure your program keeps delivering value.";
